...
contract(
  ...
  (accounts) => {
    ...
    it("the recipient should be able to withdraw from the channel", async () => {
      ...
      const claimAmnt = web3.utils.toWei("1", "ether");
      const message = web3.utils.soliditySha3(
        { t: "address", v: contractAddress },
        { t: "uint256", v: claimAmnt }
      );
      ...
      const calculatedBalance = web3.utils
        ...
        .add(web3.utils.toBN(claimAmnt))
        .sub(web3.utils.toBN(transactionFee));

      assert.equal(
        calculatedBalance,
        recBalanceAfter,
        "this is not the expected balance"
      );
    });
